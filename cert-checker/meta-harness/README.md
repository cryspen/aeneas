...
gate. The active gate after the Phase 2a Lean-emit retirement is:

- **g_rust** — source-tests-as-oracle. Runs `cargo test` against
  `--source-crate`, parses pass/fail, and maps test names to decls.

`g_lean` closes by transitivity (g_rust + soundness of the cert
walker); `g_rfl` is not implemented yet. The earlier `g_byte`
(byte-identity against mainline `aeneas -backend lean`) and `c_lean`
(does the L₁ Lean emit typecheck) gates were retired with the
Lean-emit backend in Phase 2a.
...
| `--gates g_rust`              | Comma-separated. Default: `g_rust`. Use `none` to enumerate decls only. |
...
| `--aeneas <path>`             | Override the aeneas binary. Auto-detects `src/_build/default/main.exe`. Required for `--crate` / `--llbc` modes. |
...
```toml
[gates]
g_rust = "auto"        # "skip" drops the gate; "auto" runs it (default)

[decls."crate::path::fn"]
g_rust = { skip = "api_reshape" }              # honest skip
g_rust = { divergent = "<reason>" }            # allowlist divergence
```
