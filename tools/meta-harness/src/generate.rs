...
pub fn generate_for_cert_dir(
    ...
    //
    // Mirror fallback: if `tests/src/<fx>.rs` is "polluted" but a
    // stripped sibling exists at `<out>/../../src/fixtures/<fx>.rs`
    // (i.e. `tests/lean-checker/differential/src/fixtures/<fx>.rs`),
    // use that mirror instead. The mirrors are checked in to the
    // differential crate and regenerated by
    // `scripts/regen-fixture-mirrors.sh`.
    let mirror_dir = out
        .parent()
        .and_then(|p| p.parent())
        .map(|p| p.join("src").join("fixtures"));
    let is_polluted = |t: &str| -> bool {
        t.contains("register_tool(verify)")
            || t.contains("#[verify::test]")
            || t.contains("verify::")
            || t.contains("#![feature(")
    };
    // Per-fn body check: when a mirror is used, individual fns may
    // have been stripped out (e.g. panic/unreachable test bodies that
    // would otherwise blow up proptest). For each candidate we later
    // confirm the chosen source file actually defines `pub fn <name>`.
    let fn_present = |src: &str, fn_name: &str| -> bool {
        // Match any of `[pub[(...)]] [const] [unsafe] [async] fn <name>(`
        // at the start of a line (after trimming leading whitespace).
        let needle = format!("fn {}(", fn_name);
        for line in src.lines() {
            let t = line.trim_start();
            if let Some(idx) = t.find(&needle) {
                // Make sure everything before the needle is a valid
                // sequence of fn-decl prefix tokens.
                let prefix = &t[..idx];
                let ok = prefix
                    .split_whitespace()
                    .all(|tok| matches!(
    ...
    let mut usable_fixtures: BTreeMap<String, (PathBuf, String)> = BTreeMap::new();
    for fixture in by_fixture.keys() {
        ...
        if is_polluted(&src_text) {
            // Try the mirror fallback.
            let mirror_file = mirror_dir
                .as_ref()
                .map(|d| d.join(format!("{fixture}.rs")));
            let mirror_text = match &mirror_file {
                Some(p) if p.is_file() => Some(fs::read_to_string(p).unwrap_or_default()),
                _ => None,
            };
            if let (Some(mp), Some(mt)) = (mirror_file, mirror_text) {
                if !is_polluted(&mt) {
                    usable_fixtures.insert(fixture.clone(), (mp, mt));
                    continue;
                }
            }
            for c in &by_fixture[fixture] {
                ...
                summary
                    .skip_reasons
                    .entry("source uses crate-level attrs (`#![feature]` / `register_tool(verify)`) — can't be `#[path]`-included and no stripped mirror found".into())
                    .or_default()
                    .push(c.charon_path.clone());
            }
            continue;
        }
        usable_fixtures.insert(fixture.clone(), (src_file, src_text));
    }

    // Source modules (#[path] imports), only for fixtures that survive.
    for (fixture, (src_file, _)) in &usable_fixtures {
    ...
    for (fixture, cs) in &by_fixture {
        let Some((_, src_text)) = usable_fixtures.get(fixture) else {
        ...
        for c in cs {
            // Per-fn source presence: when a mirror is in use, certain
            // fns may have been deliberately omitted (e.g. panic /
            // unreachable test bodies that would crash proptest). Treat
            // such candidates as `missing-source` rather than emitting
            // a dangling reference.
            if !fn_present(src_text, &c.fn_name) {
                summary.skipped_missing_source += 1;
                summary
                    .skip_reasons
                    .entry(format!("fn body not in source (omitted from mirror?): {fixture}::{}", c.fn_name))
                    .or_default()
                    .push(c.charon_path.clone());
                block.push_str(&format!(
                    "\n// SKIPPED {}: `{}` not defined in the resolved source file\n",
                    c.charon_path, c.fn_name,
                ));
                continue;
            }
