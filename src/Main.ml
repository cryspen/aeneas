...
let () =
  ...
  (* The timeout's signal handler must interrupt the diverging function on the
     main domain, so it requires sequential translation. *)
  if !symbolic_exec_max_seconds > 0 && !parallel then (
    log#lwarning
      (lazy
        "-symbolic-exec-timeout forces sequential translation (disabling \
         -parallel)");
    parallel := false);
