...
pub enum list_generic_GList_0<T> {
    GCons(T, Box<list_generic_GList_0<T>>),
    GNil,
}

pub struct alloc_alloc_Global_1;

pub fn impl_core_num_wrapping_add_2(p0: impl core::marker::Sized, p1: impl core::marker::Sized) -> u32 {
    unimplemented!("opaque body")
}

pub fn list_generic_glist_len_0<T>(xs: list_generic_GList_0<T>) -> Result<u32> where T: 'static {
    match xs {
    list_generic_GList_0::GCons(_, __rec_0) => {
    let v1: u32 = (list_generic_glist_len_0::<T>((*__rec_0)))?;
    Ok((impl_core_num_wrapping_add_2(v1, 1u32)))
},
    list_generic_GList_0::GNil => Ok(0u32),
}
}
