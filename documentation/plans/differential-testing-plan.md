...
- (Session 5 / Item 1) — `S3`-class globals (`X1`, `Q2`, `Q3`, `S2`, `S3`, `unwrap_y`, `YVAL`, `get_z1`, `get_z2`) now resolve to the source global rather than typed placeholders. The fix is two-sided: (a) `src/cert/LlbcJson.ml::j_place` preserves Charon's `PlaceGlobal gref` info as a new optional `"global"` field on the place JSON, threading the `crate` context down so `Print.name_to_string` can resolve the qualified name; (b) `Translate/Forward.lean::seedGlobalRefsFromBlock` walks the LLBC body before the event walker fires, seeds `vm[L] := .var gN` and emits a monadic let-bind `let gN ← <global>` for each `Assign(local L, RvRef(<place with globalName>, _))`, then re-borrow + use-passthrough clauses propagate the seed through Charon's pre-pass-inserted intermediates. Skips globals whose name contains `<` (generic-instantiation marker — the cert events don't surface the caller's generic args, so we can't emit a typed call). Shim adds `core.num.{U32,I32}.{MAX,MIN,BITS}`. ConstantsRunner gains 8 new vectors (G_lean: 224 → 232).
- (Session 5 / Item 2) — `demo.lean` wired in via a new `--skip-decl <name>` CLI flag (repeatable; threaded through `Cli.lean::findFlagsAll` to `emitTranslatedCrate`'s new `skipNames` parameter). The diff harness skips 13 emit-broken decls (`CList`'s `@[discriminant isize]` attr, `Counter` trait + `Std.Usize.Insts.DemoCounter` impl signature mismatch, closure-returning `choose`, broken `list_nth*` / `list_nth1*` / `i32_id` bodies, `use_counter`), leaving `mul2_add1`/`use_mul2_add1`/`incr`/`use_incr`/`mod_add`. DemoRunner adds 35 new vectors (G_lean: 232 → 267).

Remaining for the next session (cert-walker-level, not LeanEmit):
- Generic-aware global propagation: `use_v<T, N>` still resolves `V::LEN<T, N>` to a placeholder because the cert events don't surface the caller's instantiation. Threading generics through the seed pass (or — better — through the OCaml cert serializer, surfacing the resolved `global_decl_ref.generics`) would unblock it.
