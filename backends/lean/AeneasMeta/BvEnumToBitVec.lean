...
def realizeBvEnumToBitVec (declName : Name) : CoreM Unit := do
  ...
  -- `bv_decide` registers these three constants as *reserved names* and realises
  -- them lazily through a reserved-name action (see
  -- `Lean.Meta.Tactic.BVDecide.Normalize`). The realiser functions themselves are
  -- private to that module, so we trigger realisation by name instead. Only
  -- `enumToBitVecSuffix` is exported; the other two suffixes are inlined here.
  executeReservedNameAction (.str declName enumToBitVecSuffix)
  executeReservedNameAction (.str declName "enumToBitVec_le")
  executeReservedNameAction (.str declName "eq_iff_enumToBitVec_eq")
