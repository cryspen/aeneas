...
theorem core.slice.Slice.split_at_mut.spec {T : Type} (s : Slice T) (n : Usize) :
  ...
  · refine ⟨?_, ?_, ?_, ?_, fun s0' s1' hs0' hs1' => ?_⟩
    ...
    · split_ifs with hcond
      · exact ⟨by simp [Slice.from_val], by simp [Slice.length, List.length_append]; scalar_tac⟩
      · exfalso; apply hcond
...
def Slice.mapM  {α β} (f : α → RustM β) (x : Slice α) : RustM (Slice β) :=
  ...
  | ok xs  => ok (.from xs (List.mapM_RustM_length h ▸ x.property))
  | fail e => fail e
  | div    => div
...
def core.slice.Slice.fill {T : Type} (cloneInst : core.clone.Clone T)
  ...
  | .ok val => .ok (.from val (List.mapM_RustM_length h ▸ s.property))
  | .fail e => .fail e
  | .div => .div
