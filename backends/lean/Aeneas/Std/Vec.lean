...
def alloc.vec.FromBoxSliceVec.from {T : Type} (v : alloc.vec.Vec T) : RustM (Slice T) := ok v.slice
...
def alloc.vec.CloneVec.clone {T : Type} (cloneInst : core.clone.Clone T)
  (v : alloc.vec.Vec T) : RustM (alloc.vec.Vec T) := do
